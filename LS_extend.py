...
if sys.argv[1].isdigit():          #get which DFG to run from the terminal 
    i = int(sys.argv[1])
elif sys.argv[1] == "clearall":
    if os.path.exists("Output_Extended"):   
        shutil.rmtree("Output_Extended")
    if os.path.exists('stats_extended.csv'):
        os.remove('stats_extended.csv')
    quit()                         #quit the program if output is cleared

 
metric_selection = float(sys.argv[2])  #get metric_selection from terminal

parameter_resource_constraint = 0.5   #fix parameter_resource_constraint to 0.5
...
#-----------------------------------------------------------------------------------------------
total_energy = 0
def List_Scheduling(ops_list,m_select):    #flag is true if REST is used, otherwise flag is false
    cc = 1
    ...
    while dummy_node.schd_time == 0:
        for r in resources:                 #for each resource type
            ...
            #use a metric to select from slacks and erest, which is from 0 to 1. When m_select = 0, select base on erest. When m_select = 1, select base on slacks
            U.sort(key = lambda x: x.slacks*m_select+(1-m_select)*x.e_rest, reverse=False) 
...
# print("Latency: "+str(dummy_node.schd_time)+", total power: "+str(total_energy))


if __name__ == '__main__':          #if LS_extend.py is passed in terminal instead of get_output.py

    List_Scheduling(ops,metric_selection) 
    dp = DP(ops, dummy_node.schd_time)
    lp = LP(ops, dummy_node.schd_time)
    total_energy = dp + lp
    energy_delay_produt = total_energy * dummy_node.schd_time

    print("Latency: "+str(dummy_node.schd_time)+", total energy: "+str(total_energy) + ", energy delay product: "+str(energy_delay_produt))
